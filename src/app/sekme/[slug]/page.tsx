...
// "DD.MM.YYYY" -> epoch ms (en yeni önce sıralamak için)
function tarihToTs(tarih?: string): number {
  if (!tarih) return 0;
  const [g, a, y] = tarih.split(".");
  if (!g || !a || !y) return 0;
  return new Date(Number(y), Number(a) - 1, Number(g)).getTime();
}
